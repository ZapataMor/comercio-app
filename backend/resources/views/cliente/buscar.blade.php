...
@if ($q === '')
    <div class="bg-white rounded-2xl shadow p-8 text-center">
        ...
        <p class="text-slate-500 text-sm">Escribe algo para buscar productos. Verás primero los negocios abiertos.</p>
    </div>
@else
           ...
           class="flex items-center justify-between bg-white rounded-xl shadow-sm p-4 mb-2 hover:shadow-md transition gap-3">
            <div class="min-w-0">
                ...
                <p class="text-slate-400 text-xs mt-1">
                    🏪 {{ $p->negocio->nombre }}
                    <span class="ml-1 rounded px-1.5 py-0.5 {{ $p->negocio->activo ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                        {{ $p->negocio->activo ? 'Abierto' : 'Cerrado' }}
                    </span>
                </p>
            </div>
